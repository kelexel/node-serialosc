...
var defaultAttributes = require(__dirname+'/device.attributes.js');
...
module.exports = prime({
  ...
  _getAttributes: function(options) {
    options = options || {};
    var opts = {};
    // _.extend(opts, defaultAttributes, options);
    object.merge(opts, defaultAttributes, options);
    opts.id = nextId;
    if (!opts.listenPort) opts.listenPort = this._getRandomPort(opts);
    if (!opts.serialoscPort) opts.serialoscPort = this._getRandomPort(opts);
    return opts;
  },
