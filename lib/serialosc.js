...
function createDevice(options) {
  ...
  device.oscIn = function(msg) {
    ...
    switch(addr) {
      ...
      case '/sys/rotation':
        if (msg.length > 0) {
          device.rotation = parseInt(msg[0], 10);
        }
        device.oscOut('/sys/rotation', device.rotation);
        break;
  ...
  device.setLedState = function(x, y, s) {
    ...
    var oldX = x;
    x = device.translateX(x, y);
    y = device.translateY(oldX, y);
  ...
  device.translateX = function(x, y) {
    var newX = x;
    if (device.rotation == 90) {
      newX = y;
    }
    else if (device.rotation == 180) {
      newX = device.sizeY - y - 1;
    }
    else if (device.rotation == 270) {
      newX = device.sizeY - y - 1;
    }
    return newX;
  };

  device.translateY = function(x, y) {
    var newY = y;
    if (device.rotation == 90) {
      newY = device.sizeX - x - 1;
    }
    else if (device.rotation == 180) {
      newY = device.sizeX - x - 1;
    }
