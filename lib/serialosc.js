  ...
  device.serialoscOscIn = function(msg) {
    ...
    switch(addr) {
      ...
      case device.prefix + '/grid/led/set':
        device.setGridState(msg[0], msg[1], msg[2]);
        break;
    }
  }

  device.setGridState = function(x, y, s) {
    if (device.gridState.length != device.sizeX || (device.gridState[0] && device.gridState[0].length != device.sizeY)) {
      device.gridState = [];
      for (var gx = 0; gx < device.sizeX; gx++) {
        device.gridState[gx] = [];
        for (var gy = 0; gy < device.sizeY; gy++) {
          device.gridState[gx][gy] = 0;
        }
      }
    }
    if (device.gridState[x][y] != s) {
      device.gridState[x][y] = s;
